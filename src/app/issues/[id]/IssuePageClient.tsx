        ...
        <div className="flex items-start gap-3 py-3" style={{ borderBottom: '1px solid var(--color-base-200)' }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: 'var(--color-base-200)', color: 'var(--color-base-500)', fontFamily: "'Roboto Mono', monospace", fontSize: '0.75rem' }}>STATUS</span>
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        if (tagsOpen) { e.stopPropagation(); setTagsOpen(false); return; }
      ...
      if (e.key === 'j' || e.key === 'J') { e.preventDefault(); closeAll(); const currentIdx = [null, ...(allProjects ?? [])].findIndex(p => (p ? (p as any).id : null) === (task?.projectId ?? null)); setProjectHighlight(currentIdx >= 0 ? currentIdx : 0); setProjectOpen(true); return; }
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div className="flex items-center justify-between">
              <span className="hidden sm:inline" style={{ fontSize: '0.75rem', color: 'var(--color-base-400)', fontFamily: "'Roboto Mono', monospace" }}>
                ⌘/Ctrl+Enter to send
              </span>
              <button
                type="submit"
        ...
        <div className="flex-1 flex flex-col overflow-hidden" style={{ borderRight: '1px solid var(--color-base-200)' }}>
          ...
          <div className="flex-shrink-0 px-8 pt-6 pb-5" style={{ borderBottom: '1px solid var(--color-base-200)' }}>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span style={{ color: 'var(--color-base-400)', fontFamily: "'Roboto Mono', monospace", fontSize: '0.75rem' }}>·</span>
              <span style={{ color: 'var(--color-base-500)', fontFamily: "'Roboto Mono', monospace", fontSize: '0.75rem', fontWeight: 600 }}>{task?.issueId}</span>
              {task?.project && (
                <>
              ...
              <div className="ml-auto flex items-center gap-1.5" style={{ position: 'relative' }}>
                ...
                {editMenuOpen && (
                  <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 4, background: 'var(--color-base)', border: '1px solid var(--color-base-300)', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.12)', zIndex: 50, minWidth: 120 }}>
                    ...
                    >
                      <span>Edit</span>
                      <span style={{ fontSize: '0.75rem', color: 'var(--color-base-400)', fontFamily: "'Roboto Mono', monospace", background: 'var(--color-base-200)', border: '1px solid var(--color-base-300)', borderRadius: 3, padding: '1px 5px' }}>E</span>
                    </button>
                  </div>
                )}
              </div>
                              ...
                              return (
                                <>
                                  {allOpts.map((opt) => {
                                    ...
                                    return (
                                      <>
                                        {needsSection && myIdx === 1 && <div key="sec-me" className="px-3 pt-2 pb-0.5" style={{ color: 'var(--color-base-500)', fontSize: '0.7rem', fontFamily: "'Instrument Sans', sans-serif", textTransform: 'uppercase', letterSpacing: '0.06em' }}>Me</div>}
                                        {needsSection && myIdx > 1 && <div key="sec-agents" className="px-3 pt-2 pb-0.5" style={{ color: 'var(--color-base-500)', fontSize: '0.7rem', fontFamily: "'Instrument Sans', sans-serif", textTransform: 'uppercase', letterSpacing: '0.06em' }}>Agents</div>}
